...
Rules:
- DELETE generic/navigation records such as Download, View All, Skip to content.
...
def is_social_url(url):
    ...
    return domain in SOCIAL_DOMAINS or any(
        domain.endswith("." + d) for d in SOCIAL_DOMAINS
    )
...
def find_bad_records(rows):
    ...
    for row in rows:
        ...
        # Explicit generic/navigation titles.
        if title_norm in GENERIC_TITLES:
            reason = f"generic_title:{title_norm or 'empty'}"

        # Social/share links are not notification records.
        elif is_social_url(url):
            reason = "social_media_url"

        # Date-only titles are metadata, not notice titles.
        elif is_date_only_title(title):
            reason = "date_only_title"

        # Extremely long titles are usually page boilerplate accidentally captured.
        elif len(title) > 500:
            reason = f"suspiciously_long_title:{len(title)}"

        # Known portal/homepage entries from the audit.
        elif source == "DMER Maharashtra" and (
            title_norm in {
                "बॉण्ड सर्व्हिस पोर्टल",
