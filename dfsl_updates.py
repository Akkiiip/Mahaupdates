"""Fetch official Maharashtra DFSL recruitment and latest notifications."""
import os
...
URL='https://dfsl.maharashtra.gov.in/recruitment.html'
KEYWORDS=('recruit','advertisement','eligible','ineligible','waiting','document verification','result','exam','scientific','officer')
def main():
 ...
 for a in soup.select('a[href]'):
  ...
  seen.add(href); typ=classify(title); payload={'source':'DFSL Maharashtra','title':title,'title_english':title,'title_marathi':'','type':typ,'recruitment_title':title,'status':'pending','official_url':href,'first_seen':now,'last_seen':now,**seo_metadata('DFSL Maharashtra',title,typ)}
 ...
 print(f'DFSL sync complete: {added} new updates')
