"""Fetch official IBPS CRP and ongoing recruitment updates into MahaUpdate."""
import os
...
KEYWORDS=('crp','recruitment','probationary','management trainee','specialist officer','rrb','customer service associate','bank','officer','application','result','notice','exam','registration')
...
def main():
    ...
    for a in soup.select('a[href]'):
        ...
        if 'ibps.in' not in href: continue
        if href in seen: continue
        seen.add(href); typ=classify(title)
        payload={'source':'IBPS','title':title,'title_english':title,'title_marathi':'','type':typ,'recruitment_title':title,'status':'pending','official_url':href,'first_seen':now,'last_seen':now,**seo_metadata('IBPS',title,typ)}
    ...
    print(f'IBPS sync complete: {added} new updates')
