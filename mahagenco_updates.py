...
def classify(title):
    ...
    if "waiting" in text: return "Waiting List"
    if "selection" in text: return "Selection List"
    if "corrigendum" in text: return "Corrigendum"
    if "exam" in text: return "Exam"
    if "interview" in text: return "Interview"
    if any(word in text for word in ("advt", "advertisement", "recruit")): return "Advertisement"
    return "Notification"
...
def contextual_title(anchor):
    ...
    if row:
        ...
        for cell in row.find_all(["th", "td"]):
            ...
            text = clean(text)
            if meaningful(text): parts.append(text)
    ...
    for tag in ("li", "article", "section", "div"):
        ...
        text = re.sub(r"\b(?:download|view|click here|pdf|details)\b", "", text, flags=re.I)
        text = clean(text)
        if meaningful(text) and len(text) <= 300: return text
...
def main():
    ...
    seen = set(); added = updated = skipped_generic = 0
    for anchor in soup.select("a[href]"):
        href = urljoin(URL, anchor.get("href"))
        title = contextual_title(anchor)
        blob = f"{title} {href}".lower()
        if not title or href in seen or not any(keyword in blob for keyword in KEYWORDS): continue
        ...
        payload = {"source":"MAHAGENCO","title":title,"title_english":title,"title_marathi":"","type":classify(title),"recruitment_title":title,"status":"pending","official_url":href,"first_seen":now,"last_seen":now}
        if existing:
            client.table("updates").update({"title":title,"title_english":title,"recruitment_title":title,"type":payload["type"],"last_seen":now}).eq("official_url", href).execute(); updated += 1
        else:
